...
func (m *PostgresManager) Create(ctx context.Context, actor MutationActor, correlationID string, input CreateInput) (CreateResult, error) {
	...
	if err := tx.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM organizations WHERE id = $1::uuid)`, input.OrganizationID).Scan(&organizationExists); err != nil {
	...
	result := CreateResult{
		...
		Version: VersionResult{VersionNumber: 1, Effect: input.InitialVersion.Effect},
	}
	...
	if err := tx.QueryRow(ctx, `
		...
		RETURNING id::text
	`, result.Policy.ID, input.InitialVersion.SourceSPIFFEID, input.InitialVersion.DestinationSPIFFEID, input.InitialVersion.Action, input.InitialVersion.Effect, actor.ID, input.InitialVersion.ChangeReason).Scan(&result.Version.ID); err != nil {
...
func (m *PostgresManager) AppendVersion(ctx context.Context, actor MutationActor, correlationID, policyID string, input AppendVersionInput) (VersionResult, error) {
	...
	if err := tx.QueryRow(ctx, `
		...
		WHERE id = $1::uuid
		FOR UPDATE
	`, policyID).Scan(&organizationID, &revision); errors.Is(err, pgx.ErrNoRows) {
	...
	result := VersionResult{PolicyID: policyID, VersionNumber: nextVersion, Effect: input.Version.Effect}
	if err := tx.QueryRow(ctx, `
		INSERT INTO access_policy_versions (
			policy_id,
			version_number,
		...
		RETURNING id::text
	`, policyID, nextVersion, input.Version.SourceSPIFFEID, input.Version.DestinationSPIFFEID, input.Version.Action, input.Version.Effect, actor.ID, input.Version.ChangeReason).Scan(&result.ID); err != nil {
	...
	`, policyID, input.ExpectedRevision); err != nil {
	...
	if err := appendAudit(ctx, tx, actor, correlationID, organizationID, "access_policy.append_version", policyID, map[string]any{
		...
		"operator_role":  actor.Role,
		"policy_revision": input.ExpectedRevision + 1,
...
func (m *PostgresManager) Activate(ctx context.Context, actor MutationActor, correlationID, policyID string, input ActivateInput) (PolicyResult, error) {
	...
	if err := tx.QueryRow(ctx, `
		...
		WHERE id = $1::uuid
		FOR UPDATE
	`, policyID).Scan(&result.OrganizationID, &result.Name, &result.Status, &result.Revision, &result.ActiveVersionID); errors.Is(err, pgx.ErrNoRows) {
	...
	`, input.VersionID, policyID).Scan(&versionNumber); errors.Is(err, pgx.ErrNoRows) {
		return PolicyResult{}, ErrNotFound
	} else if err != nil {
		return PolicyResult{}, fmt.Errorf("validate policy activation version: %w", err)
	}
	...
	if command, err := tx.Exec(ctx, `
		...
		WHERE id = $1::uuid AND revision = $2
	`, policyID, input.ExpectedRevision, input.VersionID); err != nil {
